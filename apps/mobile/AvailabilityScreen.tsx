...
// 6am -> 12am (midnight). We'll render 6..23 (18 rows)
const START_HOUR = 6;
...
function startOfWeekMonday(d: Date) {
  ...
  // Convert to Monday index: Mon=1 ... Sun=0
...
type AvailabilityMap = Record<string, boolean>; // key: `${date}|${HH}:00`
...
export default function AvailabilityScreen() {
  ...
  function clearWeek() {
    // remove keys for just this week
    const weekKeys = new Set<string>();
    for (const d of days) {
      const ymd = toYMD(d);
      for (const h of hours) weekKeys.add(`${ymd}|${pad2(h)}:00`);
    }
    setSelected((prev) => {
      const next: AvailabilityMap = {};
      for (const [k, v] of Object.entries(prev)) {
        if (!weekKeys.has(k)) next[k] = v;
      }
      return next;
    });
  }
    ...
    <View style={styles.screen}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false}>
        <View>
          ...
          {/* Time rows */}
          <ScrollView style={{ maxHeight: 560 }} showsVerticalScrollIndicator>
            {hours.map((hour) => (
...
const styles = StyleSheet.create({
  screen: { flex: 1, backgroundColor: "#fff", paddingTop: 24, paddingHorizontal: 16 },
  ...
  dayHeaderText: { fontWeight: "800" },
  dayHeaderSub: { color: "#666", marginTop: 2 },

  row: { flexDirection: "row", alignItems: "center", marginBottom: 8 },
  timeCol: { width: 70, alignItems: "flex-start", paddingRight: 8 },
  timeText: { color: "#555", fontWeight: "700" },
  ...
  cellOff: { backgroundColor: "#e6e6e6", borderColor: "#cfcfcf" },
  cellOn: { backgroundColor: "#7CDE6A", borderColor: "#4FBF3D" },
  cellPressed: { transform: [{ scale: 0.98 }] },

  footerHint: { marginTop: 12 },
